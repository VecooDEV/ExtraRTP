...
public class RandomTeleportCommand {
    ...
    private static int executeDimensionPlayer(CommandSourceStack source, String dimension, ServerPlayer player) {
        ...
        if (level == null) {
            source.sendSystemMessage(UtilChat.formatMessage(ExtraRTP.getInstance().getLocale().getNotDimensionFound()
                    .replace("%dimension%", dimension)));
            return 0;
        }

        if (!Utils.randomTeleport(level, player)) {
            source.sendSystemMessage(UtilChat.formatMessage(ExtraRTP.getInstance().getLocale().getFailedTeleport()));
            return 0;
        }

        source.sendSystemMessage(UtilChat.formatMessage(ExtraRTP.getInstance().getLocale().getSuccessfulTeleportPlayer()
                .replace("%dimension%", dimension.toLowerCase())
